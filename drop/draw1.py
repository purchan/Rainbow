...
dt    = 0.001
## Gen
genL  = [vec(-60, x, 0) for x in np.arange(0,50,0.1)]
genD  = [vec(0, 0, 0)]
lisL  = [[]]
lisD  = []
...
def gatherResult(o, _str):
    lisR.append({"PS":_str, "RL": o.rl, "WL": o.λ, "E ":o.E, "Ip":o.initpos, "Cp":o.pos, "V ":o.v})
def printResult():
    for res in lisR:
        print('{')
        for key in res:
...
gr1p = graph(width=800, align='left')
gr1s = graph(width=800, align='left')
ft1p = gdots(graph=gr1p, color=lambdaToRgb(0.38), width=0.5)
ft2p = gdots(graph=gr1p, color=lambdaToRgb(0.78), width=0.5)
ft1s = gdots(graph=gr1s, color=lambdaToRgb(0.38), width=0.5)
ft2s = gdots(graph=gr1s, color=lambdaToRgb(0.78), width=0.5)

gr2p = graph(width=800, align='left')
gr2s = graph(width=800, align='left')
ft3p = gdots(graph=gr2p, color=lambdaToRgb(0.38), width=0.5)
ft4p = gdots(graph=gr2p, color=lambdaToRgb(0.78), width=0.5)
ft3s = gdots(graph=gr2s, color=lambdaToRgb(0.38), width=0.5)
ft4s = gdots(graph=gr2s, color=lambdaToRgb(0.78), width=0.5)
for res in lisR:
    if res["RL"][len(res["RL"])-1]=="r":
        if res["WL"]==0.38:
            if res["PS"]=="p":
                ft1p.plot(pos=(diff_angle(res["Ip"]-genD[0],vec(-10,0,0)), res["E "]))
                ft3p.plot(pos=(diff_angle(res["Ip"]-genD[0],vec(-10,0,0)), diff_angle(res["Cp"]-genD[0],res["V "])))
            if res["PS"]=="s":
                ft1s.plot(pos=(diff_angle(res["Ip"]-genD[0],vec(-10,0,0)), res["E "]))
                ft3s.plot(pos=(diff_angle(res["Ip"]-genD[0],vec(-10,0,0)), diff_angle(res["Cp"]-genD[0],res["V "])))
        if res["WL"]==0.78:
            if res["PS"]=="p":
                ft2p.plot(pos=(diff_angle(res["Ip"]-genD[0],vec(-10,0,0)), res["E "]))
                ft4p.plot(pos=(diff_angle(res["Ip"]-genD[0],vec(-10,0,0)), diff_angle(res["Cp"]-genD[0],res["V "])))
            if res["PS"]=="s":
                ft2s.plot(pos=(diff_angle(res["Ip"]-genD[0],vec(-10,0,0)), res["E "]))
                ft4s.plot(pos=(diff_angle(res["Ip"]-genD[0],vec(-10,0,0)), diff_angle(res["Cp"]-genD[0],res["V "])))
printResult()
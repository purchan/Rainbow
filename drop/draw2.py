...
dt    = 0.001
## Gen
genL  = [vec(-60, -50*m.cos(m.pi*x), 0) for x in [1/12, 1/6, 1/4, 1/3, 5/12, 1/2]]
genD  = [vec(0, 0, 0)]
lisL  = [[]]
lisD  = []
...
ft0p  = gdots(graph=gr1p, color=lambdaToRgb(0.41), width=0.5)
ft15p = gdots(graph=gr1p, color=lambdaToRgb(0.48), width=0.5)
ft30p = gdots(graph=gr1p, color=lambdaToRgb(0.53), width=0.5)
ft45p = gdots(graph=gr1p, color=lambdaToRgb(0.58), width=0.5)
ft60p = gdots(graph=gr1p, color=lambdaToRgb(0.61), width=0.5)
ft75p = gdots(graph=gr1p, color=lambdaToRgb(0.70), width=0.5)
fp = [ft0p, ft15p, ft30p, ft45p, ft60p, ft75p]

gr1s  = graph(width=800, align='left') 
ft0s  = gdots(graph=gr1s, color=lambdaToRgb(0.41), width=0.5)
ft15s = gdots(graph=gr1s, color=lambdaToRgb(0.48), width=0.5)
ft30s = gdots(graph=gr1s, color=lambdaToRgb(0.53), width=0.5)
ft45s = gdots(graph=gr1s, color=lambdaToRgb(0.58), width=0.5)
ft60s = gdots(graph=gr1s, color=lambdaToRgb(0.61), width=0.5)
ft75s = gdots(graph=gr1s, color=lambdaToRgb(0.70), width=0.5)
fs = [ft0s, ft15s, ft30s, ft45s, ft60s, ft75s]

for res in lisR:
    if res["RL"][len(res["RL"])-1]=="r":
        angle = round(diff_angle(res["Ip"]-genD[0],vec(-10,0,0))*12/pi)
        if res["PS"]=="p":
            fp[angle].plot(pos=(res["WL"], res["E "]))
        if res["PS"]=="s":
            fs[angle].plot(pos=(res["WL"], res["E "]))
